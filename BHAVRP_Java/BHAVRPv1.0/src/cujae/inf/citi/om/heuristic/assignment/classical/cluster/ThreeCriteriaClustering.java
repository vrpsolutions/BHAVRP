...
public class ThreeCriteriaClustering extends ByCluster{
	...
	public Solution toClustering(){
		...
		while((!listCustomersToAssign.isEmpty()) && (!listClusters.isEmpty()))
		{
			...
			{	
				...
				capacityDepot = InfoProblem.getProblem().getTotalCapacityByDepot(InfoProblem.getProblem().getDepotByIDDepot(listClusters.get(posCluster).getIDCluster()));
	...
	private ArrayList<Double> getListCriteriasByClusters(int idCustomer, ArrayList<Cluster> clusters, int criteria){
		...
		int posCustomer = InfoProblem.getProblem().getPosElement(idCustomer);
	...
	private double getAvgByCluster(int posCustomer, Cluster cluster, NumericMatrix costMatrix){
		...
		for(int i = 0; i < cluster.getItemsOfCluster().size(); i++)
			distances += costMatrix.getItem(posCustomer, InfoProblem.getProblem().getPosElement(cluster.getItemsOfCluster().get(i)));
		
		distances += costMatrix.getItem(posCustomer, InfoProblem.getProblem().getPosElement(cluster.getIDCluster()));
	...
	private double getVarByCluster(int posCustomer, Cluster cluster, NumericMatrix costMatrix){
		...
		for(int i = 0; i < cluster.getItemsOfCluster().size(); i++)
		{
			posElement = InfoProblem.getProblem().getPosElement(cluster.getItemsOfCluster().get(i));
			distance = costMatrix.getItem(posCustomer, posElement);
			difference += Math.pow(distance - avgDist, 2);
		}

		posElement = InfoProblem.getProblem().getPosElement(cluster.getIDCluster());
		distance = costMatrix.getItem(posCustomer, posElement);
		difference += Math.pow(distance - avgDist, 2);
	...
	private ArrayList<Double> getDistancesInCluster(int idCustomerRef, Cluster cluster){
		...
		int posCustomerRef = InfoProblem.getProblem().getPosElement(idCustomerRef);
		NumericMatrix costMatrix = new NumericMatrix(InfoProblem.getProblem().getCostMatrix());
		...
		for(int i = 0; i < cluster.getItemsOfCluster().size(); i++)
		{
			posCC = InfoProblem.getProblem().getPosElement(cluster.getItemsOfCluster().get(i));
			listDistCluster.add(costMatrix.getItem(posCustomerRef, posCC));
		}

		if(listDistCluster.isEmpty())
		{
			posCC = InfoProblem.getProblem().getPosElement(cluster.getIDCluster());
			listDistCluster.add(costMatrix.getItem(posCustomerRef, posCC));
		}

...
public abstract class Partitional extends Assignment {
	...
	protected ArrayList<Integer> generateElements(SeedType seedType, DistanceType distanceType){
		...
		switch(distanceType.ordinal())
		{
			...
			{
				try {
					costMatrix = Problem.getProblem().fillCostMatrix(Problem.getProblem().getCustomers(), Problem.getProblem().getDepots(), distanceType);
	...
	protected ArrayList<Integer> generateElements(ArrayList<Customer> partition, DistanceType distanceType){
		ArrayList<Integer> idElements = new ArrayList<Integer>();
		
		int totalCustomers = partition.size();
		int totalDepots = Problem.getProblem().getTotalDepots();
		int counter = totalDepots;
		...
		switch(distanceType.ordinal())
		{
			...
			{
				try {
					costMatrix = Problem.getProblem().fillCostMatrix(partition, Problem.getProblem().getDepots(), distanceType);
				} catch (IllegalArgumentException | SecurityException
						| ClassNotFoundException | InstantiationException
						| IllegalAccessException | InvocationTargetException
						| NoSuchMethodException e) {
		...
		while(counter > 0)
		{
			...
			idElement = partition.get(rcBestAll.getCol()).getIDCustomer();	
			idElements.set((rcBestAll.getRow() - totalCustomers), idElement);	
	...
	protected ArrayList<Integer> generateElements(DistanceType distanceType){
		...
		Depot depotCentral = new Depot();
		depotCentral.setIDDepot(-1);
		depotCentral.setLocationDepot(centralCoordinate());
		
		ArrayList <Depot> listDepotCentral = new ArrayList<Depot>();
		listDepotCentral.add(depotCentral);
		...
		switch(distanceType.ordinal())
		{
			...
			{
				try {
					costMatrix = Problem.getProblem().fillCostMatrix(Problem.getProblem().getCustomers(), listDepotCentral, distanceType);
...
//			}
		...
		while(counter > 0)
		{
			rcBestAll = costMatrix.indexBiggerValue(totalCustomers, 0, (totalCustomers + 1 - 1), (totalCustomers - 1));
	...
	protected ArrayList<Integer> generateElements(){

		DistanceType distanceType = DistanceType.Euclidean; // se pasa por parámetro y por defecto se coloca la euclidea
	...
	protected double calculateCost(ArrayList<Cluster> clusters, NumericMatrix costMatrix, ArrayList<Depot> medoids, ArrayList<Customer> listPartition) {
		...
		for(int i = 0; i < clusters.size(); i++) 
		{			
			posDepot = Problem.getProblem().getPosElement(medoids.get(i).getIDDepot(), listPartition);
			listIDCustomers = clusters.get(i).getItemsOfCluster();
			...
			{	
				idCustomer =  listIDCustomers.get(j);				
				posCustomer = Problem.getProblem().getPosElement(idCustomer, listPartition);
	...
	protected double calculateDissimilarity(DistanceType distanceType, ArrayList<Cluster> clusters)
	{
		...
		for(int i = 0; i < clusters.size(); i++)
		{
			...
			{
				int temp = Problem.getProblem().getPosElement(clusters.get(i).getItemsOfCluster().get(j));
				for(int k = (j + 1); k < clusters.get(i).getItemsOfCluster().size(); k++)
				{
					currentDissimilarity += dissimilarityMatrix.getItem(temp, Problem.getProblem().getPosElement(clusters.get(i).getItemsOfCluster().get(k)));
				}
			}
		}
		
		currentDissimilarity /= clusters.size();
	...
	private Location centralCoordinate(){
		...
		double currentAxisX = 0.0;
		double currentAxisY = 0.0;
		...
		{
			currentAxisX += listCoordinatesCustomers.get(i).getAxisX();
			currentAxisY += listCoordinatesCustomers.get(i).getAxisY();
		}

		currentAxisX /= listCoordinatesCustomers.size();
		currentAxisY /= listCoordinatesCustomers.size();

		Location centralCoordinate = new Location(currentAxisX, currentAxisY);
	...
	protected ArrayList<Integer> sortedElements (ArrayList<Integer> idElements, DistanceType distanceType){
		...
		ArrayList<Integer> organizedElements = new ArrayList<Integer>();
		ArrayList<Customer> customers = new ArrayList<Customer>();

		for(int i = 0; i < idElements.size(); i++)
		{
			organizedElements.add(-1);
...
//			}
		...
		while(j < idElements.size())
		{
			...
			organizedElements.set((rcBestAll.getCol() - idElements.size()), idElements.get(rcBestAll.getRow()));
			j++;
		}
	...
	protected ArrayList<ArrayList<Customer>> generatePartitions(int sampsize){

...
public class Modified_KMEANS extends Partitional {
	...
	public Solution toClustering() {
		...
		ArrayList<Depot> centroids = replicateDepots(Problem.getProblem().getDepots());
		...
		do {	
			...
			try 
			{
				costMatrix = Problem.getProblem().fillCostMatrix(listCustomersToAssign, centroids, distanceType);
			} 
			...
			if(change)
			{
				...
				try				
				{
					costMatrix = Problem.getProblem().fillCostMatrix(Problem.getProblem().getCustomers(), centroids, distanceType);
				} 
				catch (IllegalArgumentException | SecurityException
						| ClassNotFoundException | InstantiationException
						| IllegalAccessException | InvocationTargetException

package cujae.inf.citi.om.heuristic.assignment;


import java.util.ArrayList;

import cujae.inf.citi.om.heuristic.controller.Tools;
...
public abstract class Assignment {
	...
	protected ArrayList<Cluster> initializeClusters(){
		...
		for(int i = 0; i < totalClusters; i++)
		{
			...
			cluster = new Cluster(InfoProblem.getProblem().getListIDDepots().get(i).intValue(), 0.0, listIDItems); 
	...
	protected ArrayList<Cluster> initializeClusters(ArrayList<Integer> listIDElements){
		...
		if(totalElements == InfoProblem.getProblem().getDepots().size())
		{
			for(int i = 0; i < totalElements; i++)
			{
				listIDCustomers = new ArrayList<Integer>();
				listIDCustomers.add(listIDElements.get(i).intValue());
				cluster = new Cluster(InfoProblem.getProblem().getDepots().get(i).getIDDepot(), InfoProblem.getProblem().getRequestByIDCustomer(listIDElements.get(i).intValue()), listIDCustomers);
		...
		else // UPGMC
		{
			for(int i = 0; i < totalElements; i++)
			{
				...
				if(InfoProblem.getProblem().findPosDepot(InfoProblem.getProblem().getDepots(), listIDElements.get(i).intValue()) == -1)
				{
					listIDCustomers.add(listIDElements.get(i).intValue());
					cluster = new Cluster(listIDElements.get(i).intValue(), InfoProblem.getProblem().getRequestByIDCustomer(listIDElements.get(i).intValue()), listIDCustomers);
				}
	...
	protected int getPosCluster(int posCustomer, ArrayList<Cluster> clusters){
		int posCluster = -1;
		int idCustomer = InfoProblem.getProblem().getListIDCustomers().get(posCustomer);
		int i = 0;
		int j;
		boolean found = false;
	...
	protected Location recalculateTest(Cluster clusterOne, Cluster clusterTwo, ArrayList<Customer> customersToAssign) {
		...
		locationOne = customersToAssign.get(InfoProblem.getProblem().findPosCustomer(customersToAssign, clusterOne.getIDCluster())).getLocationCustomer();
		locationTwo = customersToAssign.get(InfoProblem.getProblem().findPosCustomer(customersToAssign, clusterTwo.getIDCluster())).getLocationCustomer();
	...
	protected Location recalculateWeightCentroid(Cluster clusterOne, Cluster clusterTwo, ArrayList<Customer> customersToAssign) {
		Location locationOne = new Location();
		Location locationTwo = new Location();
		Location location = new Location();
		...
		aveAxisX = (locationOne.getAxisX() * clusterOne.getItemsOfCluster().size()) + (locationTwo.getAxisX() * clusterTwo.getItemsOfCluster().size());
		aveAxisY = (locationOne.getAxisY() * clusterOne.getItemsOfCluster().size()) + (locationTwo.getAxisY() * clusterTwo.getItemsOfCluster().size());

		aveAxisX /= (clusterOne.getItemsOfCluster().size() + clusterTwo.getItemsOfCluster().size());
		aveAxisY /= (clusterOne.getItemsOfCluster().size() + clusterTwo.getItemsOfCluster().size());
	...
	protected Location recalculateCentroid(Cluster cluster) {
		...
		{	
			Location location = new Location();
			location = InfoProblem.getProblem().getLocationByIDCustomer(cluster.getItemsOfCluster().get(i)); 
